...
static void wavelet_noise_worker(greyscale ** base, int size, int base_size)
{
    for(int x = 0; x < base_size; x++)
        for(int y = 0; y < base_size; y++)
            (*base)->data[x][y] = ((float)rand()) / RAND_MAX * 3 - 1;

    greyscale scaled = **base;
    scaled.downscale(base_size / 2);
    scaled.upscale(base_size);
    **base -= scaled;
...
static void set_wavelet_noise_iteration(void ** input)
{
    greyscale ** base = ((greyscale**)(input[0]));
    int size = *((int*)(input[1]));
    int detail = *((int*)(input[2]));

    wavelet_noise_worker(base, size, detail);
}

greyscale PTG::Noises::wavelet(int size, float * distribution)
{
    ...
    for(int i = 0; i < iterations; i++)
    {
        if(distribution[i] != 0)
        {
            threader.tasks[k] = set_wavelet_noise_iteration;
...
greyscale PTG::Noises::wavelet(int size, float distribution_coef)
{
    int iterations = log2(size);
    ...
    return Noises::wavelet(size, distribution);
}
